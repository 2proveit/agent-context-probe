      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-gray-500">
            ...
            {session.children?.length ? <span>{formatCount(session.children.length, 'subagent')}</span> : null}
    ...
    >
      ...
      >
        <span className="flex h-7 w-7 shrink-0 items-center justify-center text-gray-500">
          {reasoning ? <Brain className="h-4 w-4" /> : <MessageSquareText className="h-4 w-4" />}
        </span>
        <span className="min-w-0 flex-1">
          <span className="flex min-w-0 items-baseline gap-2">
            <span className="shrink-0 text-sm font-medium text-gray-900">{title}</span>
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-sm text-gray-500">{modelLabel(summary.model)}</span>
      ...
      <div className={child ? 'px-5 py-5' : ''}>
        {prompt ? (
          <div className="mb-4 rounded-xl border border-gray-200 bg-stone-50 px-4 py-3">
            ...
            <p className="whitespace-pre-wrap break-words text-sm leading-6 text-gray-700">{prompt}</p>
          </div>
        ) : null}
        ...
        <div className="relative space-y-1 pl-5 before:absolute before:bottom-2 before:left-[7px] before:top-2 before:w-px before:bg-gray-200">
          {detail.requests.map((rawRequest, index) => {
            ...
            const response = normalizeAssistantResponse(request.response?.body, request.endpoint);
            ...
            return (
              <div key={request.requestId} className="relative pb-3">
                ...
                <div className="space-y-1">
                  ...
                  {!reasoning && !text && tools.length === 0 && !request.response?.streamError ? (
                    <TimelineItem
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {new Date(detail.summary.firstTimestamp).toLocaleString()} · {formatCount(detail.summary.children?.length ?? detail.children?.length ?? 0, 'subagent')}
                    </p>
                  </div>
